...
function listFilm(request, response) {
  ...
  console.log("0. ListFilms - Listar películas")
  filmBussines
  .listFilm()
  .then(function(film){
...
function findFilm(request, response) {
  console.log("1. FindFilm - Encuentra la película")
...
function insertFilm(request, response) {
  console.log("2. insertFilm - Inserta película, logica de negocio")
...
function modifyFilm(request, response) {
  console.log("3. Modify Film - Modifica la película")
  let id = request.url.split("/").pop()
  request.on("data", function(bodyContent) {
      let film = JSON.parse(bodyContent)
      film._id = id
...
function deleteFilm(request, response) {
  console.log("4. DeleteFilm - Borrar película")
  let id = request.url.split("/").pop()
